...
Cloudflare universal middleware supports `website`, `api`, `bypassPaths`, and per-hostname `multidomainConfig`. At least one of `website`, `api`, or `multidomainConfig` is required.
...
- `bypassPaths` are matched before `api.basePaths` and pass through without a lock check.
- `api` applies an API-only quarantine response under the listed base paths.
- `website` applies the lock page and CSP to HTML traffic.
- `multidomainConfig` lets different hostnames use different options on the same Worker.
...
### CRITICAL Forgetting to provide at least one of website, api, or multidomainConfig
