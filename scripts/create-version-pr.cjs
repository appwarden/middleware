...
async function createVersionPR() {
  try {
    ...
    // Commit the changes
    exec(`git commit -m "chore(release): update version to ${newVersion} [skip ci]"`);
    ...
    exec(`echo "${process.env.GITHUB_TOKEN}" | gh auth login --with-token`);
    ...
    exec(`gh pr create --base main --head ${branchName} --title "${prTitle}" --body "${prBody}" --label "dependencies"`);
