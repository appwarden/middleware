...
function parseArgs() {
  ...
  for (const arg of args) {
    ...
    if (cwdMatch) result.cwd = path.resolve(cwdMatch[1])
...
function safeWriteFile(filePath, content, cwd) {
  try {
    ...
    // Guard against hard-link attacks: refuse to overwrite a target that is
    // linked to multiple inodes (could redirect the write to another file).
    if (fs.existsSync(target)) {
      const targetStat = fs.lstatSync(target)
      if (targetStat.nlink > 1) {
        warn(`Refusing to write through hard link: ${target}`)
        return false
      }
    }
    ...
    // Final verification: catch symlink races that occurred during the write window.
